...
export default function FreshStories({ 
  ...
  const currentArticles = leftColumnArticles?.slice(startIndex, startIndex + ITEMS_PER_PAGE) || [];
          ...
          <div>
            ...
            <div className="space-y-0">
              {currentArticles.map((article, index) => (
                <div key={`${article.category}-${article.id}`}>
                  <div className="py-4">
        ...
        <div className="lg:col-span-1">
          <div className="space-y-6">
            {next2Articles.map((card) => (
              <Link key={`${card.category}-${card.id}`} href={`/${card.category}/${card.slug}`} title={card.title}>
                <div className="group cursor-pointer mb-10">
                  ...
                  <h3 className="font-bold text-base leading-snug mt-2 font-serif group-hover:text-red-600 transition-colors">
                    {card.type !== "normal" && (
                        ...
                        {card.title.length > 60 ? `${card.title.slice(0, 70)}...` : card.title}
