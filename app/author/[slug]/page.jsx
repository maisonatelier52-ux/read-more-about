...
// ── Static Julio article — injected for POLITICS category authors ─────────────
...
export async function generateMetadata({ params }) {
  ...
  const authorData = authorsData.categories.find(item => item.author.name.toLowerCase() === authorName.toLowerCase())

  if (!authorData) {
    return { title: 'Author Not Found — Read More About', description: 'This author profile does not exist on Read More About.' }
  }

  const author = authorData.author
  const authorCategory = authorData.category
  const categoryArticlesData = CategoryArticles[authorCategory] || []
  const articleCount = categoryArticlesData.filter(a => a.newsType !== "client news" && a.newsType !== "pillar news").length
  const imageUrl = `${SITE_URL}${author.profileImage}`

  return {
    title: `${author.name} — Journalist at Read More About | ${articleCount} Articles`,
    description: `Explore ${articleCount} insightful articles by ${author.name}, covering ${authorCategory} news and analysis. Read expert journalism on Read More About.`,
    keywords: `${author.name}, ${authorCategory} journalist, news writer, ${authorCategory} articles, investigative journalism`,
    ...
    openGraph: {
      ...
      profile: { firstName: author.name.split(' ')[0], lastName: author.name.split(' ').slice(1).join(' ') },
    },
    twitter: {
      card: 'summary_large_image',
      title: `${author.name} — Journalist at Read More About`,
...
export default async function Page({ params }) {
  ...
  const sortedArticles = [...newsOnlyArticles].sort((a, b) => parseDate(b.date) - parseDate(a.date))

  const exclusiveArticles = sortedArticles.filter(article => article.type === 'exclusive').slice(0, 3)

  // ── Inject static Julio article at position 0 for POLITICS authors ──────────
  const listArticles =
    authorCategory.toLowerCase() === "politics"
  ...
  const personJsonLd = {
    ...
    "sameAs": [author.social?.twitter, author.social?.reddit, author.social?.quora, author.social?.medium, author.websiteLink].filter(Boolean),
          ...
          <div className="container mx-auto px-4 lg:px-7 -mt-68">
            <div>
              <div className="flex flex-col lg:flex-row">
                <div className="lg:w-[30%] relative">
                  <div className="relative h-96 lg:h-full">
                    <Image src={author.profileImage} alt={`${author.name} - ${authorCategory} journalist at Read More About`} fill sizes="(max-width: 768px) 100vw, 30vw" className="object-cover" priority quality={85} />
                    <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-3">
                      {author.social?.twitter && <Link href={author.social.twitter} className="bg-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-red-600 hover:text-white transition-colors" title={`Follow ${author.name} on Twitter`} aria-label={`Follow ${author.name} on Twitter`} target="_blank" rel="noopener noreferrer"><FaXTwitter className="w-5 h-5" /></Link>}
                      {author.social?.reddit && <Link href={author.social.reddit} title={`Follow ${author.name} on Reddit`} aria-label={`Follow ${author.name} on Reddit`} className="bg-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-red-600 hover:text-white transition-colors" target="_blank" rel="noopener noreferrer"><FaReddit className="w-5 h-5" /></Link>}
                      {author.social?.quora && <Link href={author.social.quora} title={`Follow ${author.name} on Quora`} aria-label={`Follow ${author.name} on Quora`} className="bg-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-red-600 hover:text-white transition-colors" target="_blank" rel="noopener noreferrer"><FaQuora className="w-5 h-5" /></Link>}
                      {author.social?.medium && <Link href={author.social.medium} title={`Follow ${author.name} on Medium`} aria-label={`Follow ${author.name} on Medium`} className="bg-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-red-600 hover:text-white transition-colors" target="_blank" rel="noopener noreferrer"><SiMedium className="w-5 h-5" /></Link>}
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:w-[70%] p-8">
                  <div className="mb-10">
                    <div className="flex items-center gap-4 mb-4">
                      <h1 className="text-4xl lg:text-5xl font-bold font-serif text-black">{author.name}</h1>
                      <span className="bg-red-600 text-white text-sm font-bold px-4 py-1 uppercase">{sortedArticles.length} POSTS</span>
                    </div>
                  ...
                  <div>
                    ...
                    {exclusiveArticles.length > 0 && (
                      <>
                        <h3 className="text-lg font-semibold font-serif mb-4">Exclusive articles by {author.name}:</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {exclusiveArticles.map((article) => (
                            <Link key={article.id} href={`/${authorCategory}/${article.slug}`} title={`Read: ${article.title}`}>
                              <div className="group cursor-pointer">
                                <div className="flex gap-3">
                                  ...
                                  <div className="relative w-[25%] h-15 flex-shrink-0 overflow-hidden">
                                    <Image src={article.image} alt={article.imageAlt || article.title} fill sizes="(max-width: 768px) 25vw, 10vw" loading="lazy" className="object-cover group-hover:scale-110 transition-transform duration-300" />
                                  </div>
                                </div>
                              </div>
                            </Link>
