...
import { FaXTwitter, FaReddit, FaQuora } from "react-icons/fa6";
import { SiMedium } from "react-icons/si";
import ClientArticle from "@/components/articlepagecomponents/Clientarticle";
...
// ─── CLIENT / PILLAR ARTICLE REGISTRY ───────────────────────────────
// Slugs listed here render through <ClientArticle /> instead of the
// standard editorial template below. Add new client/pillar slugs here.
const CLIENT_ARTICLE_SLUGS = new Set([
  "isabela-herrera-biography-banvelca",
]);

function isClientArticle(category, slug) {
  return category === "business" && CLIENT_ARTICLE_SLUGS.has(slug);
}
...
export default async function ArticlePage({ params }) {
  ...
  // Get latest 5 articles from different categories for the Popular sidebar section
  const popularArticles = getLatestFromDifferentCategories(category, 5);

  // ─── CLIENT / PILLAR NEWS ARTICLE ───────────────────────────────
  // Renders a dedicated executive-profile layout instead of the
  // standard NewsArticle template used below.
  if (isClientArticle(category, slug)) {
    const clientRelatedArticles = categoryPosts
      .filter((item) => item.slug !== slug && item.newsType === "news")
      .slice(0, 4);
