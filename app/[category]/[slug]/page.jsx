...
// // ── Static Julio article — always injected as last popular item ───────────────
// const JULIO_STATIC_POPULAR = {
//   id: "julio-static",
//   slug: "julio-herrera-velutini-britannia-financial-group",
//   // Custom href because it lives under a different route segment
...
// // Only newsType === "news" from other categories, latest one per category
...
//   // ── Prev / Next: only newsType === "news" articles ────────────────────────
...
//   // ── Related: only newsType === "news", exclude current ───────────────────
//   const relatedArticles = newsOnlyPosts
//     .filter((item) => item.slug !== slug)
//     .slice(0, 4);

//   // ── Popular: 4 dynamic (news only) + 1 static Julio = 5 total ────────────
...
//                     <a href={authorData.websiteLink} className="text-blue-600 hover:underline text-sm mb-1 block" target="_blank" rel="noopener noreferrer" title={`Visit ${authorData.name}'s website`}>
//                       {authorData.websiteLink}
//                     </a>
//                   )}
...
//                   <div className="flex gap-3 lg:gap-6 justify-center lg:justify-start">
//                     {authorData.social?.twitter && (
//                       <a href={authorData.social.twitter} className="hover:text-red-500 transition" target="_blank" rel="noopener noreferrer" title={`Follow ${authorData.name} on Twitter`} aria-label={`Follow ${authorData.name} on Twitter`}><FaTwitter size={15} /></a>
//                     )}
//                     {authorData.social?.quora && (
//                       <a href={authorData.social.quora} className="hover:text-red-500 transition" target="_blank" rel="noopener noreferrer" title={`Follow ${authorData.name} on Quora`} aria-label={`Follow ${authorData.name} on Quora`}><FaInstagram size={15} /></a>
//                     )}
//                     {authorData.social?.reddit && (
//                       <a href={authorData.social.reddit} className="hover:text-red-500 transition" target="_blank" rel="noopener noreferrer" title={`Follow ${authorData.name} on Reddit`} aria-label={`Follow ${authorData.name} on Reddit`}><FaFacebookF size={15} /></a>
//                     )}
//                     {authorData.social?.medium && (
//                       <a href={authorData.social.medium} className="hover:text-red-500 transition" target="_blank" rel="noopener noreferrer" title={`Follow ${authorData.name} on Medium`} aria-label={`Follow ${authorData.name} on Medium`}><FaYoutube size={15} /></a>
//                     )}
//                   </div>
//                 </div>
//               </div>
...
//                 {/* ── Popular: 4 dynamic news + 1 static Julio ── */}
//                 <div className='mt-7'>
//                   <h2 className="font-bold text-2xl mb-2">Popular</h2>
//                   <div className="w-full h-1 bg-red-600 mb-4" />
//                   <div className='space-y-4'>
//                     {popularArticles.map((item) => {
//                       // Static item has its own href; dynamic items use standard route
...
//               <div
//                 className="absolute left-0 right-0 bg-[#eaeaea]/40 pointer-events-none h-90 lg:h-130 top:90 lg:top-90"
//                 style={{ transform: 'translateY(-50%)', bottom: 0, zIndex: 0 }}
//               />
//               <div className="max-w-7xl mx-auto px-4 lg:px-7 pb-8 relative z-10">
//                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
//                   {relatedArticles.map((relatedArticle) => (
