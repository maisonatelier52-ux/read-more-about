...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    creator: SITE_NAME,
    publisher: SITE_NAME,
    alternates: { canonical: `${SITE_URL}/${categoryKey}` },
    openGraph: {
      title: `${formattedCategory} — Breaking News & Expert Analysis`,
      description,
      url: `${SITE_URL}/${categoryKey}`,
      ...
      type: "website",
      locale: "en_US",
      images: [{ url: `${SITE_URL}/images/read-more-about-logo.webp`, width: 1200, height: 630, alt: `${formattedCategory} News - Read More About` }],
    },
    twitter: {
      card: "summary_large_image",
      title: `${formattedCategory} — Breaking News & Expert Analysis`,
...
export default async function Page({ params }) {
  ...
  if (!categoryArticlesData) notFound()
  ...
  const latestArticle = sortedArticles[0]
  const baseListArticles = sortedArticles.slice(1)

  // ── Inject static Julio article at position 0 of listArticles for POLITICS ──
  const listArticles =
    categoryKey === "politics"
      ? [JULIO_STATIC_ARTICLE, ...baseListArticles]
      : baseListArticles
    ...
    "mainEntity": {
      ...
      "itemListElement": sortedArticles.slice(0, 10).map((article, index) => ({
        ...
        "item": {
          ...
          "image": article.image ? `${SITE_URL}${article.image}` : `${SITE_URL}/images/read-more-about-logo.webp`,
          "mainEntityOfPage": { "@type": "WebPage", "@id": `${SITE_URL}/${categoryKey}/${article.slug}` },
        },
      })),
    },
    "publisher": {
  ...
  const webPageJsonLd = {
    ...
    "about": { "@type": "Thing", "name": formattedCategory },
    "primaryImageOfPage": { "@type": "ImageObject", "url": `${SITE_URL}/images/read-more-about-logo.webp` },
  };
    ...
    <>
      ...
      <div className="relative mb-30">
        <section className="sr-only">
          ...
          <h2>Latest {formattedCategory} Headlines and Breaking News</h2>
          <p>Browse our collection of {sortedArticles.length} {formattedCategory.toLowerCase()} articles.</p>
        </section>
          ...
          <div className="container mx-auto px-4 pt-12">
            <nav aria-label="Breadcrumb" className="mb-6">
              <ol className="flex items-center gap-2 text-sm">
                ...
                <li>/</li>
                <li className="text-gray-600 capitalize">{formattedCategory}</li>
              </ol>
            </nav>
            <h2 className="text-5xl font-bold text-black text-center mb-16 font-serif">{formattedCategory}</h2>
          </div>
        </div>
          ...
          {latestArticle && (
            <div className="mb-12">
              <Link href={`/${category}/${latestArticle.slug}`} title={latestArticle.title}>
                <article className="group cursor-pointer">
                  <div className="relative h-[350px] lg:h-[450px] overflow-hidden shadow-2xl rounded-lg">
                    <Image src={latestArticle.image} alt={latestArticle.imageAlt || latestArticle.title} fill sizes="100vw" className="object-cover transition-transform duration-700" priority />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent" />
                    <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-12 max-w-4xl">
                      <div className="flex gap-3 mb-4">
                        <span className="bg-red-600 text-white text-xs lg:text-sm font-bold px-2 lg:px-4 py-2 uppercase tracking-wide">{latestArticle.category}</span>
                        {latestArticle.type !== "normal" && (
                          <span className="bg-white text-black text-xs lg:text-sm font-bold px-2 lg:px-4 py-2 uppercase tracking-wide">{latestArticle.type}</span>
                        )}
                      </div>
                      <h3 className="text-white font-bold text-md lg:text-2xl leading-tight mb-4 group-hover:text-red-400 transition-colors duration-300">{latestArticle.title}</h3>
                      <p className="text-gray-200 text-xs lg:text-md leading-relaxed mb-6 max-w-3xl">
                        {latestArticle.excerpt.length > 150 ? `${latestArticle.excerpt.slice(0, 150)}...` : latestArticle.excerpt}
                      </p>
                      <div className="flex items-center gap-4 text-sm text-gray-300">
                        {authorInfo && (
                          <div className="flex items-center gap-2">
                            <div className="relative w-9 h-9 rounded-full overflow-hidden">
                              <Image src={authorInfo.profileImage} alt={authorInfo.name} fill sizes="36px" className="object-cover" />
                            </div>
                            <span className="font-medium text-xs">{authorInfo.name}</span>
                          </div>
                        )}
                        <span>-</span>
                        <time dateTime={parseDate(latestArticle.date).toISOString()}>{formatDate(latestArticle.date)}</time>
                      </div>
                    </div>
                  </div>
                </article>
