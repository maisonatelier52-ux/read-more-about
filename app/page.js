...
// Slug of the article that must always appear in the FreshStories "next2Articles" slot
const PINNED_NEXT2_SLUG = "isabela-herrera-biography-banvelca";
...
// Helper function to find a specific article by its slug
...
function processArticlesData() {
  ...
  // ── PINNED ARTICLE: force this slug into next2Articles, and never let it repeat elsewhere ──
  const pinnedArticle = findArticleBySlug(sortedArticles, PINNED_NEXT2_SLUG);

  let freshStoriesNext2 = [];
  if (pinnedArticle && !usedArticleIds.has(`${pinnedArticle.category}-${pinnedArticle.id}`)) {
    // Reserve the pinned article's id immediately so it can't be picked again anywhere else
    usedArticleIds.add(`${pinnedArticle.category}-${pinnedArticle.id}`);
    freshStoriesNext2.push(pinnedArticle);

    // Fill the remaining slot(s) with the next unique article(s)
    const remainingSlots = 2 - freshStoriesNext2.length;
    if (remainingSlots > 0) {
      freshStoriesNext2 = freshStoriesNext2.concat(
        getUniqueArticles(sortedArticles, remainingSlots, false, usedArticleIds)
      );
    }
  } else {
    // Pinned article not found, or already used (e.g. it was the "latest" article) —
    // fall back to normal behavior without repeating it.
    freshStoriesNext2 = getUniqueArticles(sortedArticles, 2, false, usedArticleIds);
  }
